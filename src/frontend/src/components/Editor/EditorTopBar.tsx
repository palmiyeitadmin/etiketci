        ...
        <header className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-[color:var(--plms-border)] bg-[color:var(--plms-panel)] px-4 py-3 xl:px-6">
            ...
            <div className="flex min-w-0 flex-wrap items-center justify-end gap-3">
                <div className="flex items-center gap-2 rounded-2xl border border-[color:var(--plms-border)] bg-[color:var(--plms-panel-2)] px-2 py-2">
                    <EditorCommandButton icon={ArrowCounterClockwise} label="Undo" onClick={onUndo} disabled={!canUndo} />
                    <EditorCommandButton icon={ArrowClockwise} label="Redo" onClick={onRedo} disabled={!canRedo} />
                    <EditorCommandButton icon={ArrowUDownLeft} label="Rotate Left" onClick={onRotateLeft} />
                    <EditorCommandButton icon={ArrowUDownRight} label="Rotate Right" onClick={onRotateRight} />
                    <EditorCommandButton icon={Copy} label="Duplicate" onClick={onDuplicate} />
                    <EditorCommandButton icon={Trash} label="Delete" onClick={onDelete} tone="danger" />
                </div>
                <div className="flex items-center gap-2 rounded-2xl border border-[color:var(--plms-border)] bg-[color:var(--plms-panel-2)] px-2 py-2">
                    ...
                    <button className="inline-flex min-w-0 items-center gap-2 rounded-xl bg-blue-600 px-3 py-2 text-[10px] font-black uppercase tracking-[0.18em] text-white transition-colors hover:bg-blue-700" type="button" onClick={onSave}>
                        ...
                        <span className="hidden xl:inline">{isDirty ? "Save Draft" : "Saved"}</span>
                    </button>
                </div>
            </div>
        </header>
